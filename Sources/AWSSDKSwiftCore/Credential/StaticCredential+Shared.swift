...
public struct ConfigFileCredentialProvider: CredentialProviderWrapper {

    /// Errors occurring when initializing a ConfigFileCredentialProvider
    ///
    ...
    public func getProvider(httpClient: AWSHTTPClient, on eventLoop: EventLoop) -> CredentialProvider {
        return ConfigFileCredentialProviderWorker(profile: profile, credentialsFilePath: credentialsFilePath, on: eventLoop)
    }
}
...
/// - missingAccessKeyId: If the access key ID was not found
/// - missingSecretAccessKey: If the secret access key was not found
internal class ConfigFileCredentialProviderWorker: CredentialProvider {
    var credential: Credential?
    var setupPromise: EventLoopPromise<Credential>
    
    init(profile: String, credentialsFilePath: String = "~/.aws/credentials", on eventLoop: EventLoop) {
    ...
    static func sharedCredentials(from byteBuffer: ByteBuffer, for profile: String) throws -> StaticCredential {
        ...
        do {
            ...
            throw ConfigFileCredentialProvider.ConfigFileCredentialError.invalidCredentialFileSyntax
        }
        
        guard let config = parser.sections[profile] else {
            throw ConfigFileCredentialProvider.ConfigFileCredentialError.missingProfile(profile)
        }
        
        guard let accessKeyId = config["aws_access_key_id"] else {
            throw ConfigFileCredentialProvider.ConfigFileCredentialError.missingAccessKeyId
        }
        
        guard let secretAccessKey = config["aws_secret_access_key"] else {
            throw ConfigFileCredentialProvider.ConfigFileCredentialError.missingSecretAccessKey
        }
