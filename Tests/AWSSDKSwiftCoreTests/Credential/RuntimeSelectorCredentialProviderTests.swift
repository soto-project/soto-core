...
class RuntimeSelectorCredentialProviderTests: XCTestCase {
    func testSetupFail() {
        ...
        let futureResult = client.credentialProvider.getCredential(on: client.eventLoopGroup.next(), context: TestEnvironment.context)
    ...
    func testFoundEnvironmentProvider() throws {
        ...
        defer { XCTAssertNoThrow(try client.syncShutdown()) }
        let futureResult = client.credentialProvider.getCredential(on: client.eventLoopGroup.next(), context: TestEnvironment.context).flatMapThrowing { credential in
    ...
    func testConfigFileProviderFail() {
        ...
        defer { XCTAssertNoThrow(try client.syncShutdown()) }
        let futureResult = client.credentialProvider.getCredential(on: client.eventLoopGroup.next(), context: TestEnvironment.context).flatMapThrowing { _ in
            let internalProvider = try XCTUnwrap((client.credentialProvider as? RuntimeSelectorCredentialProvider)?.internalProvider)
            XCTAssert(internalProvider is StaticCredential)
