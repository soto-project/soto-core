...
final class AWSSignerTests: XCTestCase {
    ...
    func testSignPutHeaders() {
        ...
        let headers = signer.signHeaders(url: URL(string: "https://sns.eu-west-1.amazonaws.com/")!, method: .POST, headers: ["Content-Type": "application/x-www-form-urlencoded; charset=utf-8"], body: "Action=ListTopics&Version=2010-03-31".utf8, date: Date(timeIntervalSinceReferenceDate: 200))
    ...
    func testSignS3PutURL() {
        ...
        let url = signer.signURL(url: URL(string: "https://test-bucket.s3.amazonaws.com/test-put.txt")!, method: .PUT, body: "Testing signed URLs".utf8, date: Date(timeIntervalSinceReferenceDate: 100000))
    ...
    func testBodyData() {
        ...
        let signer = AWSSigner(credentials: credentials, name: "sns", region:"eu-west-1")
        let headers1 = signer.signHeaders(url: URL(string: "https://sns.eu-west-1.amazonaws.com/")!, method: .POST, body: string.utf8, date: Date(timeIntervalSinceReferenceDate: 0))
        let headers2 = signer.signHeaders(url: URL(string: "https://sns.eu-west-1.amazonaws.com/")!, method: .POST, body: data, date: Date(timeIntervalSinceReferenceDate: 0))
        let headers3 = signer.signHeaders(url: URL(string: "https://sns.eu-west-1.amazonaws.com/")!, method: .POST, body: buffer.readableBytesView, date: Date(timeIntervalSinceReferenceDate: 0))
    ...
    func testPerformanceSignedHeaders() {
        ...
        measure {
            for _ in 0..<1000 {
                _ = signer.signHeaders(url: URL(string: "https://test-bucket.s3.amazonaws.com/test-put.txt")!, method: .GET, headers: ["Content-Type": "application/x-www-form-urlencoded; charset=utf-8"], body: string.utf8)
            }
        }
    }
